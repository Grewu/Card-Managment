package org.card.controller.filter;
...
import org.card.validation.JwtTokenValidation;
...
public class JwtFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    var email = jwtTokenValidation.getEmail(token);

    var grantedAuthorities =
        jwtTokenValidation.getAuthorities(token).stream().map(SimpleGrantedAuthority::new).toList();

    var authenticated =
        UsernamePasswordAuthenticationToken.authenticated(email, null, grantedAuthorities);
